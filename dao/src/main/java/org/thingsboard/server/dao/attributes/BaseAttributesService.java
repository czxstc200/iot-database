...
public class BaseAttributesService implements AttributesService {
    ...
    public ListenableFuture<List<Void>> save(EntityId entityId, String scope, List<AttributeKvEntry> attributes) {
        ...
        for (AttributeKvEntry attribute : attributes) {
            ...
            if(attribute.getKey().equals("hardware_id")){
                device.setHardwareId(attribute.getStrValue().get());
            }
            else if(attribute.getKey().equals("status")){
                device.setStatus(attribute.getBooleanValue().get());
            }
            else if(attribute.getKey().equals("online")){
                device.setOnline(attribute.getBooleanValue().get());
            }
            else if(attribute.getKey().equals("serial_number")){
                device.setSerialNumber(attribute.getStrValue().get());
            }
            else if(attribute.getKey().equals("ip")){
                device.setIp(attribute.getStrValue().get());
            }
        }
