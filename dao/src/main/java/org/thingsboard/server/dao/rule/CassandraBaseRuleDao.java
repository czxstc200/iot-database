...
import static org.thingsboard.server.dao.model.ModelConstants.RULE_BY_SENSOR_ID;
import static org.thingsboard.server.dao.model.ModelConstants.RULE_SENSOR_ID;
...
public class CassandraBaseRuleDao extends CassandraAbstractSearchTextDao<RuleMetaDataEntity, RuleMetaData> implements RuleDao {
    ...
    @Override
    public List<RuleMetaData> findRulesByDeviceId(String device_id) {
        log.debug("Search rules by device_id [{}]", device_id);
        Select select = select().from(ModelConstants.RULE_BY_DEVICE_ID);
        Select.Where query = select.where();
        query.and(eq(ModelConstants.RULE_DEVICE_ID, device_id));
        return DaoUtil.convertDataList(findListByStatement(query));
    }

    @Override
    public Optional<RuleMetaData> findRuleBySensorId(UUID sensor_id) {
        Select select = select().from(RULE_BY_SENSOR_ID);
        Select.Where query = select.where();
        query.and(eq(RULE_SENSOR_ID, sensor_id.toString()));
