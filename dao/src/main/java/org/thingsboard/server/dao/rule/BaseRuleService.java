...
public class BaseRuleService extends AbstractEntityService implements RuleService {
    ...
    public RuleMetaData saveRule(RuleMetaData rule) {
        ...
        rule.setDevice_id("cfa99ea0-ba1c-11e7-9224-b15ed33c0abd");
        DeviceId deviceId = new DeviceId(UUID.fromString(rule.getDevice_id()));
        Device device = deviceService.findDeviceById(deviceId);
        rule.setDevice_name(device.getName());
        rule.setSensor_id("temperature");
        rule.setSensor_name(device.getSensor().get("temperature").get("sensor_name").asText());
    ...
    @Override
    public List<RuleMetaData> findRuleByDeviceId(String device_id) {
        //validateId(ruleId, "Incorrect rule id for search rule request.");
        return ruleDao.findRulesByDeviceId(device_id);
    }

    @Override
    public Optional<RuleMetaData> findRuleBySensorId(UUID sensor_id) {
        //validateId(ruleId, "Incorrect rule id for search rule request.");
        return ruleDao.findRuleBySensorId(sensor_id);
    }
