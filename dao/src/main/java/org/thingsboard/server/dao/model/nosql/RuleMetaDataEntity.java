...
public class RuleMetaDataEntity implements SearchTextEntity<RuleMetaData> {
    ...
    @Column(name = RULE_DEVICE_ID)
    private String device_id;
    @Column(name = RULE_DEVICE_NAME)
    private String device_name;
    @Column(name = RULE_SENSOR_ID)
    private String sensor_id;
    @Column(name = RULE_SENSOR_NAME)
    private String sensor_name;
    @Column(name = RULE_THRESHOLD_TYPE)
    private String threshold_type;
    @Column(name = RULE_UPPER_LIMIT)
    private Double upper_limit;
    @Column(name = RULE_LOWER_LIMIT)
    private Double lower_limit;
    @Column(name = RULE_ALARM_STATUS)
    ...
    public RuleMetaDataEntity(RuleMetaData rule) {
        ...
        this.device_id = rule.getDevice_id();//"d4328050-b575-11e7-8670-1567cc345bca";
        /*DeviceId deviceId = new DeviceId(UUID.fromString(this.device_id));
        DeviceService deviceService = new DeviceServiceImpl();
        Device device = deviceService.findDeviceById(deviceId);*/
        this.device_name = rule.getDevice_name();
        this.sensor_id = rule.getSensor_id();
        this.sensor_name = rule.getSensor_name();
        this.threshold_type = "NUMERICAL";
        if(this.threshold_type.equals("NUMERICAL")) {
            this.upper_limit = 25.0;
            this.lower_limit = 15.0;
            this.alarm_status = null;
        }else if(this.threshold_type.equals("ON_OFF")) {
    ...
    public Double getUpper_limit() { return upper_limit; }
    ...
    public RuleMetaData toData() {
        ...
        rule.setDevice_id(device_id);
        rule.setDevice_name(device_name);
        rule.setSensor_id(sensor_id);
        rule.setSensor_name(sensor_name);
        rule.setThreshold_type(threshold_type);
        rule.setUpper_limit(upper_limit);
        rule.setLower_limit(lower_limit);
        rule.setAlarm_status(alarm_status);
        return rule;
    }
